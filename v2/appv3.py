# -*- coding: utf-8 -*-
"""
Created on Tue Sep 14 22:25:14 2021
...
from asyncio import new_event_loop, set_event_loop, run_coroutine_threadsafe, Queue, LifoQueue, create_task, gather, sleep
...
from tkinter import StringVar, IntVar, Canvas, Text, Tk, Entry, Button, Frame, Label
...
parser.add_argument('-o', '--openPositions', nargs='*', action='append')
...
class TheWindow:
    ...
    def destroyWelcomeFrame2(self, prevFrames):
        ...
        for b in bases:
            ...
            for dictionary in all_filters:
                ...
                maxQuote.append(temp * filters[p]['maxQty'])
                min_trade_allocation.append(filters[p]['minNotional'] * (100 / 97))
                
        maxQuote = max(maxQuote)
        min_trade_allocation = min(min_trade_allocation)
    ...
    def setPairs(self):
        ...
        pairsListbox = Listbox(pairsFrame, selectmode=SINGLE, bd=4)
    ...
    def mainWindow(self):
        ...
        buyButton = Button(
            ...
            borderwidth=2,
            highlightthickness=0,
            command=lambda: self.threads[baseChoice.get()].manualBuy(),
            font=("Roboto", int(18)),
            relief="ridge"
        )
        buyButton.place(
    ...
    def startAsync(self, frames, sliders):
        self.threads = {}
        for b in bases:
            self.threads[b] = AsyncioThread(self, b)
        for b in bases:
            self.threads[b].start()
    ...
    def refresh_frames(self, frames, sliders):
        ...
        while True:
            for b in bases:
                if not self.threads[b].price.empty():
                    frames['prices'][b].configure(state='normal')
                    frames['prices'][b].insert("end", str(self.threads[b].price.get_nowait()) + '\n')
                    frames['prices'][b].see("end")
                    frames['prices'][b].yview("end")
                    frames['prices'][b].configure(state='disabled')
                if not self.threads[b].trades.empty():
                    ...
                    frames['trades'].insert("end", self.threads[b].trades.get_nowait())
                    frames['trades'].see("end")
                    frames['trades'].yview("end")
                    frames['trades'].configure(state='disabled')
                if not self.threads[b].charts.empty():
                    for w in frames['chart'][b].frame.winfo_children():
                        w.destroy()
                    figs = self.threads[b].charts.get_nowait()
                    for fig in figs:
                        frm = Frame(frames['chart'][b].frame, 
                                    bg = "#FFFFFF", 
                                    width = 550, 
                                    height = 200, 
                                    bd=0
                ...
                if not self.threads[b].open_positions_display.empty():
                    pos = self.threads[b].open_positions_display.get_nowait()
                    if pos[0] == 'add':
                        frames['positions'].insert("end", f"[{pos[2]}] {pos[1]} {b}")
                    elif pos[0] == 'del':
                        frames['positions'].delete(pos[1])
                if not self.threads[b].errors.empty():
                    e = self.threads[b].errors.get_nowait()
                    messagebox.showerror(
                        title="Error",
                        message=str(e) + "\nAutoTrader will now restart.")
                    restartQ.put_nowait(1)
...
class AsyncioThread(Thread):
    ...
    async def kline_data(self):
        ...
        ks = bm.kline_socket(self.pair, interval=KLINE_INTERVAL_1MINUTE)
    ...
    async def get_historical_data(self):
        ...
        klines = array(await self.client.get_historical_klines(self.pair, AsyncClient.KLINE_INTERVAL_1MINUTE, '1 day ago UTC')).astype(npdouble)
    ...
    def update_df(self, df, new_row):
        ...
        if strategy == 1:
            ...
            df['EMA_26'][-200:] = df['Close'][-200:].ewm(span=26, adjust=False).mean()
            df['EMA_100'][-200:] = df['Close'][-200:].ewm(span=100, adjust=False).mean()
            df['MACD'][-1] = df['EMA_12'][-1] - df['EMA_26'][-1]
            ...
            if s == 1:                
                try:
                    ...
                    if balance < min_trade_allocation:
                        ...
                        if trade_allocation >= filters['minNotional']:
                            quantity =  trade_allocation / current_price
                        else:
    ...
    async def buy(self, qty, current_price, count):
        ...
        try:
            ...
            while True:
                await sleep(0.5)
                try:
                    trade_status = await self.client.get_order(symbol=self.pair, orderId=order['orderId'])
                except Exception as ex:
                    self.asyncio_loop.call_soon_threadsafe(self.errors.put_nowait, str(ex))
                    return False
                else:
                    if float(trade_status['executedQty']) == qty:
                        await self.save_trades('buy', qty, current_price, count, self.base)
                        ...
                        temp = curr_open_positions.get()
                        temp.append({'stop-loss': (1 - (sl / 100)) * current_price, 
                                     'take-profit': (1 + (tp / 100)) * current_price, 
                                     'qty': qty, 
                                     'id': count, 
                                     'time': strftime('%H:%M'), 
        ...
        while not close:
            ...
            for s in sell_list:
                if current_price < s['stop-loss'] or current_price > s['take-profit']:
                    ...
                    for t in temp:
                        if t['id'] == s['id']:
                            done = await self.sell(s['qty'], current_price, s['id'], temp.index(t))
                    if done:
                        sell_list.remove(s)
                        await self.open_positions.get()
                        await self.open_positions.put(sell_list)
    ...
    async def sell(self, qty, current_price, count, idx):
        ...
        try:
            ...
            await sleep(0.5)
            try:
                await self.client.get_order(symbol=self.pair, orderId=order['orderId'])
            except Exception as ex:
                self.asyncio_loop.call_soon_threadsafe(self.errors.put_nowait, str(ex))
                return False
            else:
                ...
                self.asyncio_loop.call_soon_threadsafe(self.open_positions_display.put_nowait, 
                                       ['del', idx])
                await self.save_trades('sell', qty, current_price, count, self.base)
                return True
    ...
    async def close_positions(self):
        ...
        if not res == []:
            self.asyncio_loop.call_soon_threadsafe(self.trades.put_nowait, f'Closing {self.base} positions. \n\n')
        positions = await self.open_positions.get()
        data = await self.client.get_symbol_ticker(symbol=self.pair)
        price = float(data['price'])
        temp = curr_open_positions.get()
        for i in positions:
            for t in temp:
                if i['id'] == t['id']:
                    await self.sell(i['qty'], price, i['id'], temp.index(t))
        curr_open_positions.put_nowait(temp)
    ...
    async def save_trades(self, trade_type, qty, price, count, b):
        ...
        currentBaseBalance = await self.client.get_asset_balance(asset=base)
    ...
    async def restartProgram(self):
        ...
        if useTestnet:
            command = command + '-t '
        command = command + f'-a '
        os.system('AutoTrader.exe {API_KEY} {SECRET_KEY} {sl} {tp} {trade_allocation} {useClosingTime} {startTime} {closingTime} {strategy} {quote} '
                  f'{useTestnet}, '
                  f'{API_KEY} '
                  f'{SECRET_KEY} '
                  f'{base} '
