...
if not args.restart:
    ...
    trade_count = qQueue()
    trade_count.put_nowait(args.all[10])
    bases = args.bases
    curr_open_positions = qQueue()
    l = []
    dictionary = {}
...
class TheWindow:
    ...
    def startAsync(self, frames, sliders):
        ...
        try:
            ...
            self.thread.restartQ.put_nowait(1)
    ...
    def on_closing(self):
        if messagebox.askokcancel("Quit", "Do you want to quit? "):
            self.thread.stopQ.put_nowait(1)
    ...
    def refresh_frames(self, frames, sliders):
        ...
        while True:
            ...
            while not self.thread.charts.empty():
                l = self.thread.charts.get_nowait()
                figs = l[:-1]
                b = l[-1].removesuffix(quote)
...
class AsyncioThread(Thread):
    ...
    async def checkQuit(self):
        while self.stopQ.empty() and not close:
            await sleep(2)
    ...
    async def checkRestart(self):
        while self.restartQ.empty() and not close:
            await sleep(2)
        if not self.restartQ.empty():
            self.restartQ.get()
    ...
    async def get_historical_data(self):
        ...
        for b in bases:
            p = b + quote
            klines = array(await self.client.get_historical_klines(p, AsyncClient.KLINE_INTERVAL_1MINUTE, '1 day ago UTC')).astype(npdouble)
    ...
    async def lineplots(self):
        ...
        while not close:
            [df, p] = await self.data_frame_figure.get()
            x = df['Close Time'].to_numpy()
            x = x[-60:]
            x = vecfunc(x)
    
            fig1 = Figure()
            plot1 = self.plotPrice(df, x, fig1, p)
            if strategy == 0:
                self.asyncio_loop.call_soon_threadsafe(self.charts.put_nowait, [fig1, p])
            elif strategy == 1:
                ...
                self.asyncio_loop.call_soon_threadsafe(self.charts.put_nowait, [fig1, fig2, p])
            elif strategy == 4:
                plot1.legend(loc="upper right", fontsize="x-small")
                fig2 = Figure()
                self.plotMACD(df, x, fig2)
                ...
                self.asyncio_loop.call_soon_threadsafe(self.charts.put_nowait, [fig1, fig2, fig3, p])
            elif strategy == 5:
                plot1.legend(loc="upper right", fontsize="x-small")
                fig2 = Figure()
                self.plotMACD(df, x, fig2)
                ...
                self.asyncio_loop.call_soon_threadsafe(self.charts.put_nowait, [fig1, fig2, fig3, fig4, p])
            elif strategy == 6:
                y1 = df['SMA_20'].to_numpy()
                y2 = df['SMA_50'].to_numpy()
                plot1.plot(x, y1[-60:], "-b", label="20 S.M.A.")
    ...
    def plotPrice(self, df, x, figure, p):
